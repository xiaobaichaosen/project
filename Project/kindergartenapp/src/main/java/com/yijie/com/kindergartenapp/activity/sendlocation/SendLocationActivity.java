package com.yijie.com.kindergartenapp.activity.sendlocation;
...
import com.yijie.com.kindergartenapp.activity.sendlocation.adapter.AddressAdapter;
import com.yijie.com.kindergartenapp.activity.sendlocation.utils.DataConversionUtils;
import com.yijie.com.kindergartenapp.activity.sendlocation.utils.DatasKey;
import com.yijie.com.kindergartenapp.activity.sendlocation.utils.OnItemClickLisenter;
import com.yijie.com.kindergartenapp.activity.sendlocation.utils.SPUtils;
                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
                            SPUtils.putString(SendLocationActivity.this, DatasKey.LOCATION_INFO, gson.toJson(location));
...
//                        Toast.makeText(MainActivity.this, "搜索", Toast.LENGTH_SHORT).show();
                        startActivityForResult(new Intent(SendLocationActivity.this, SearchActivity.class), SEARCHREQUESTCODE);
                        break;
...
//                        Toast.makeText(MainActivity.this, "定位", Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.bt_send:
                        if (null != mList && 0 < mList.size() && null != mAddressAdapter) {
                            ...
                            schoolAdress.setName(poiItem.getSnippet());
                            schoolAdress.setLon( poiItem.getLatLonPoint().getLongitude()+"");
                            schoolAdress.setLat(poiItem.getLatLonPoint().getLatitude()+"");
                            EventBus.getDefault().post(schoolAdress);
                            finish();
//                            Toast.makeText(SendLocationActivity.this, "发送：" + poiItem.getTitle() + "  " + poiItem.getSnippet() + "  " + "纬度：" + poiItem.getLatLonPoint().getLatitude() + "  " + "经度：" + poiItem.getLatLonPoint().getLongitude(), Toast.LENGTH_SHORT).show();
                        }
                        break;
                }
            }
...
//        mTransAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
    ...
    private void doWhenLocationSucess() {
        ...
        doSearchQuery(false, "", location.getCity(), new LatLonPoint(location.getLatitude(), location.getLongitude()));
