...
public class StepView extends View {
  ...
  public StepView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    textSize = typedArray.getDimension(R.styleable.SlidingTabs_step_textSize, 30);
    ...
    circleColor = typedArray.getColor(R.styleable.SlidingTabs_circleColor,
            ContextCompat.getColor(context, R.color.stepColor));
    grayColor = ContextCompat.getColor(context, R.color.app_background);
    radius = typedArray.getDimension(R.styleable.SlidingTabs_circleRadius, 30);
    paint = new Paint(Paint.ANTI_ALIAS_FLAG);
    linePain = new Paint(Paint.ANTI_ALIAS_FLAG);
    linePain.setStrokeWidth(8);
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (i <step-1) {
        linePain.setColor(circleColor);
      } else {
        linePain.setColor(grayColor);
      }

      textX = circleX - textBound.width() / 2;
      canvas.drawCircle(circleX, circleY, radius, paint);
      if (i==3){
        canvas.drawText("完成", textX- textBound.width()* 3/ 2 , textY, textPaint);
...
//          canvas.drawText(text, textX, textY, textPaint);
      ...
      if (!isLast){//
          ...
          canvas.drawLine(circleX+radius ,circleY,(marginLeft + radius) + perWidth * (i+1)+radius,circleY,linePain);
